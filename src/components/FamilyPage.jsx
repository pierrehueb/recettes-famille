...
export default function FamilyPage() {
  ...
  return <section className="family-section">
    ...
    <section className="family-members-section">
      ...
      </article>)}{isAdmin && <button type="button" className="member-card member-add-card" onClick={() => { setAdminOpen(true); window.setTimeout(() => document.getElementById('family-invite-card')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0) }}><span className="member-add-icon">＋</span><span><strong>Inviter un membre</strong><small>Ajouter un proche à cette famille</small></span></button>}</div>
