...
export default function FamilyPage() {
  ...
  return <section className="family-section">
    ...
    <section className="family-members-section">
      <div className="family-section-heading"><div><p className="section-kicker">Les membres</p><h3>Ceux qui font vivre ce livre</h3></div>{isAdmin && <div className="family-heading-actions"><button type="button" className="primary-button small-button" onClick={() => { setAdminOpen(true); window.setTimeout(() => document.getElementById('family-invite-card')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 0) }}>＋ Inviter un membre</button><button type="button" className="family-admin-toggle" onClick={() => setAdminOpen(value => !value)}>{adminOpen ? 'Fermer la gestion' : 'Gérer la famille'}</button></div>}</div>
