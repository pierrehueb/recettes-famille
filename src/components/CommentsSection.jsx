      ...
      <div className="comments-list">
        {comments.length === 0 ? <div className="comments-empty">Aucun souvenir n’a encore été partagé. Soyez le premier à raconter l’histoire de cette recette.</div> : comments.map(comment => {
          ...
          return <article className="comment-card" key={comment.id} style={{ position: 'relative' }}>
            ...
            {isEditing ? <><textarea value={editingText} onChange={e => setEditingText(e.target.value)} rows="4" maxLength="2000" disabled={isBusy} style={{ width: '100%', marginTop: 12 }} /><div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, marginTop: 10 }}><label style={photoActionStyle}><span aria-hidden="true">{comment.photo && !removeEditingPhoto ? '↻' : '+'}</span>{editingPhoto ? 'Changer' : comment.photo && !removeEditingPhoto ? 'Remplacer' : 'Ajouter une photo'}<input type="file" accept="image/*" disabled={isBusy} style={{ display: 'none' }} onChange={e => { setEditingPhoto(e.target.files?.[0] || null); if (e.target.files?.[0]) setRemoveEditingPhoto(false) }} /></label>{editingPhoto && <><span style={{ color: '#756a61', fontSize: '.8rem' }}>{editingPhoto.name}</span><button type="button" style={photoActionStyle} onClick={() => setEditingPhoto(null)} disabled={isBusy}>× Annuler</button></>}{comment.photo && !editingPhoto && <button type="button" style={photoActionStyle} onClick={() => setRemoveEditingPhoto(value => !value)} disabled={isBusy} aria-label={removeEditingPhoto ? 'Conserver la photo' : 'Supprimer la photo'}><span aria-hidden="true">{removeEditingPhoto ? '↶' : '⌫'}</span>{removeEditingPhoto ? 'Conserver' : 'Supprimer'}</button>}</div><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 10 }}><button type="button" className="secondary-button small-button" onClick={cancelEditing} disabled={isBusy}>Annuler</button><button type="button" className="primary-button" onClick={() => saveEdit(comment)} disabled={isBusy || !editingText.trim()}>{isBusy ? 'Enregistrement…' : 'Enregistrer'}</button></div></> : <p>{comment.content}</p>}
