...
function MediaSection({ recipeId, user }) {
  ...
  const loadMedia = async () => {
    ...
    try {
      ...
      setMembership(membership); setFamilyId(membership.family_id)
      ...
      const signed = await Promise.all((data ?? []).map(async item => { const { data: signedData } = await supabase.storage.from('family-media').createSignedUrl(item.storage_path, 3600); return { ...item, signedUrl: signedData?.signedUrl || '' } }))
      setPhotos(signed.filter(item => item.media_type === 'photo')); setOriginals(signed.filter(item => item.media_type === 'scan'))
  ...
  const uploadFiles = async (event, kind) => {
    ...
    try {
      for (const file of files) {
        ...
        const id = crypto.randomUUID(); const folder = kind === 'photo' ? `recipes/${recipeId}/photos` : `originals/${recipeId}`; const path = `${familyId}/${folder}/${id}${extension}`
        const { error: uploadError } = await supabase.storage.from('family-media').upload(path, file, { contentType: file.type || undefined, upsert: false }); if (uploadError) throw uploadError
        const { error: mediaError } = await supabase.from('media').insert({ family_id: familyId, recipe_id: recipeId, storage_path: path, media_type: kind === 'photo' ? 'photo' : 'scan', mime_type: file.type || null, original_filename: file.name, position: kind === 'photo' ? photos.length : 0, created_by: membership.id }); if (mediaError) throw mediaError
      }
      setMessage(kind === 'photo' ? 'Photo(s) ajoutée(s) à la recette.' : 'Le manuscrit original a été conservé dans le livre familial.'); await loadMedia()
  ...
  return <section className="media-section">
    ...
    {error && <div className="form-error media-message">{error}</div>}{message && <div className="media-success">{message}</div>}
...
function RecipeDetailPage({ recipeId, user, onBack }) {
  const [recipe, setRecipe] = useState(null); const [version, setVersion] = useState(null); const [isFavorite, setIsFavorite] = useState(false); const [favoriteLoading, setFavoriteLoading] = useState(false); const [ingredients, setIngredients] = useState([]); const [steps, setSteps] = useState([]); const [loading, setLoading] = useState(true); const [switchingVersion, setSwitchingVersion] = useState(false); const [error, setError] = useState('')
  const loadVersion = async versionId => {
    const { data: versionData, error: versionError } = await supabase.from('recipe_versions').select('id, version_name, notes, is_original, based_on_version_id, created_at, updated_at').eq('id', versionId).eq('recipe_id', recipeId).single(); if (versionError) throw versionError
    const [ingredientsResult, stepsResult] = await Promise.all([supabase.from('ingredients').select('id, position, quantity, unit, name, notes').eq('version_id', versionData.id).order('position', { ascending: true }), supabase.from('preparation_steps').select('id, position, instruction, duration_minutes, temperature_celsius').eq('version_id', versionData.id).order('position', { ascending: true })])
    if (ingredientsResult.error) throw ingredientsResult.error; if (stepsResult.error) throw stepsResult.error; return { version: versionData, ingredients: ingredientsResult.data ?? [], steps: stepsResult.data ?? [] }
  }
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (isFavorite) { const { error } = await supabase.from('favorites').delete().eq('user_id', user.id).eq('recipe_id', recipeId); if (error) throw error; setIsFavorite(false) }
      else { const { error } = await supabase.from('favorites').insert({ user_id: user.id, recipe_id: recipeId, family_id: recipe.family_id }); if (error) throw error; setIsFavorite(true) }
    } catch (favoriteError) { console.error('Erreur favori:', favoriteError); setError(favoriteError.message || 'Impossible de modifier les favoris.') }
    finally { setFavoriteLoading(false) }
  }
  useEffect(() => { let cancelled = false; const load = async () => { if (!supabase || !recipeId) return; setLoading(true); setError(''); try { const { data: recipeData, error: recipeError } = await supabase.from('recipes').select('id, family_id, title, description, original_author, origin_year, difficulty, servings, created_at').eq('id', recipeId).single(); if (recipeError) throw recipeError; const { data: versionData, error: versionError } = await supabase.from('recipe_versions').select('id').eq('recipe_id', recipeId).eq('is_original', true).order('created_at', { ascending: true }).limit(1).maybeSingle(); if (versionError) throw versionError; if (!versionData) throw new Error('Cette recette ne contient pas encore de version originale.'); const selected = await loadVersion(versionData.id); if (!cancelled) { setRecipe(recipeData); setVersion(selected.version); setIngredients(selected.ingredients); setSteps(selected.steps) } } catch (loadError) { if (!cancelled) setError(loadError.message || 'Impossible de charger cette recette.') } finally { if (!cancelled) setLoading(false) } }; load(); return () => { cancelled = true } }, [recipeId])
  ...
  return <section className="detail-section">
    ...
    <article className="recipe-detail">
      ...
      <button type="button" className={`favorite-button ${isFavorite ? 'is-favorite' : ''}`} onClick={toggleFavorite} disabled={favoriteLoading} aria-label={isFavorite ? 'Retirer des favoris' : 'Ajouter aux favoris'}>{isFavorite ? '♥' : '♡'}<span>{isFavorite ? 'Retirer des favoris' : 'Ajouter aux favoris'}</span></button>
      {error && <div className="form-error">{error}</div>}{switchingVersion && <div className="status-card">Chargement de la version sélectionnée…</div>}
      ...
      <MediaSection recipeId={recipeId} user={user} /><RecipeVariants recipeId={recipeId} user={user} currentVersionId={version?.id} onSelectVersion={handleSelectVersion} />
    </article>
  </section>
}
...
function AddRecipePage({ user, onCancel, onCreated }) {
  const [form, setForm] = useState({ title: '', description: '', original_author: '', origin_year: '', difficulty: '', servings: '' }); const [ingredients, setIngredients] = useState([emptyIngredient()]); const [steps, setSteps] = useState([emptyStep()]); const [loading, setLoading] = useState(false); const [error, setError] = useState('')
...
function App() {
  ...
  const loadRecipes = async () => { if (!supabase) { setRecipesError('La connexion Supabase n’est pas configurée.'); return }; setRecipesLoading(true); setRecipesError(''); const { data, error } = await supabase.from('recipes').select('id, title, description, original_author, origin_year, difficulty, servings, created_at').order('created_at', { ascending: false }); if (error) { setRecipesError(error.message); setRecipes([]) } else setRecipes(data ?? []); setRecipesLoading(false) }
  ...
  return <div className="app">
    ...
    <main className="main-content">
      ...
      {activePage === 'family' && <FamilyPage />}
      {activePage === 'favorites' && <FavoritesPage user={user} onOpen={openRecipe} />}
